...
1. **First pass (capture)** — `show table` intercepts each table in the body content and pushes it into `PAR_BUFFER` as a table item (marked with `is_table: true`), alongside paragraph items.
2. **Second pass (render)** — When iterating `PAR_BUFFER`, table items are rendered via `render-memo-table()` instead of going through paragraph numbering.
